...
test("desktop bridge exposes an audio picker for tts voice samples and manual audio", async () => {
  ...
  assert.match(mainSource, /ta-huo:select-jianying-drafts-root/)
  ...
  assert.match(preloadSource, /selectJianyingDraftsRoot/)
  assert.match(preloadSource, /ta-huo:select-jianying-drafts-root/)
  ...
  assert.match(themeProviderSource, /selectJianyingDraftsRoot/)
  assert.match(themeProviderSource, /filePath/)
  assert.match(themeProviderSource, /directory/)
})
